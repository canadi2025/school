import React, { useState, useEffect } from 'react';
import { apiService } from '../services/api';
import { Car } from '../types';
import DataTable from './DataTable';
import PlusIcon from './icons/PlusIcon';
import { useTranslation } from '../i18n/i18n';
import MaintenanceModal from './MaintenanceModal';
import InspectionModal from './InspectionModal';
import ArrowLeftIcon from './icons/ArrowLeftIcon';
import CarForm from './CarForm';

interface CarManagementProps {
  onBack?: () => void;
}

const CarManagement: React.FC<CarManagementProps> = ({ onBack }) => {
  const [cars, setCars] = useState<Car[]>([]);
  const [loading, setLoading] = useState(true);
  const { t } = useTranslation();
  const [view, setView] = useState<'list' | 'form'>('list');
  const [isMaintenanceModalOpen, setIsMaintenanceModalOpen] = useState(false);
  const [isInspectionModalOpen, setIsInspectionModalOpen] = useState(false);
  const [selectedVehicle, setSelectedVehicle] = useState<Car | null>(null);

  const fetchCars = async () => {
    try {
      setLoading(true);
      const data = await apiService.getCars();
      setCars(data);
    } catch (error) {
      console.error("Failed to fetch cars", error);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    if (view === 'list') {
      fetchCars();
    }
  }, [view]);

  const columns: { key: keyof Car; header: string }[] = [
    { key: 'id', header: t('carManagement.columns.id') },
    { key: 'make', header: t('carManagement.columns.make') },
    { key: 'model', header: t('carManagement.columns.model') },
    { key: 'year', header: t('carManagement.columns.year') },
    { key: 'licensePlate', header: t('carManagement.columns.licensePlate') },
    { key: 'status', header: t('carManagement.columns.status') },
  ];

  const handleEdit = (car: Car) => {
    alert(t('carManagement.editAlert', { make: car.make, model: car.model }));
  };

  const handleDelete = (car: Car) => {
    if (window.confirm(t('carManagement.deleteConfirm', { make: car.make, model: car.model }))) {
      alert(t('carManagement.deleteAlert', { make: car.make, model: car.model }));
    }
  };

  const handleViewMaintenance = (car: Car) => {
    setSelectedVehicle(car);
    setIsMaintenanceModalOpen(true);
  };

  const handleViewInspection = (car: Car) => {
    setSelectedVehicle(car);
    setIsInspectionModalOpen(true);
  };
  
  const handleCloseMaintenanceModal = () => {
    setIsMaintenanceModalOpen(false);
    setSelectedVehicle(null);
  }
  
  const handleCloseInspectionModal = () => {
    setIsInspectionModalOpen(false);
    setSelectedVehicle(null);
  };

  const handleMaintenanceAdded = () => {
      fetchCars();
  }

  const handleAddCar = async (carData: Omit<Car, 'id'>) => {
    try {
      await apiService.addCar(carData);
      setView('list');
    } catch (error) {
      console.error("Failed to add car", error);
      alert("Failed to add car.");
    }
  };

  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          {onBack && (
            <button 
              onClick={onBack} 
              className="p-2 text-gray-500 dark:text-gray-400 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700"
              aria-label={t('vehicleManagement.backToVehicles')}
            >
              <ArrowLeftIcon />
            </button>
          )}
          <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100">{t('carManagement.title')}</h1>
        </div>
        {view === 'list' ? (
            <button onClick={() => setView('form')} className="flex items-center px-4 py-2 font-medium text-white bg-[rgb(var(--color-primary-600))] rounded-lg hover:bg-[rgb(var(--color-primary-500))] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[rgb(var(--color-primary-600))] shadow">
                <PlusIcon />
                <span className="ml-2">{t('carManagement.addCar')}</span>
            </button>
        ) : (
            <button onClick={() => setView('list')} className="flex items-center px-4 py-2 font-medium text-white bg-gray-600 rounded-lg hover:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-600 shadow">
                <ArrowLeftIcon />
                <span className="ml-2">{t('studentManagement.backToList')}</span>
            </button>
        )}
      </div>

      {view === 'list' ? (
        <DataTable
          columns={columns}
          data={cars}
          loading={loading}
          onEdit={handleEdit}
          onDelete={handleDelete}
          onViewMaintenance={handleViewMaintenance}
          onViewInspection={handleViewInspection}
        />
      ) : (
        <CarForm onSubmit={handleAddCar} onCancel={() => setView('list')} />
      )}


      {selectedVehicle && (
          <MaintenanceModal 
            isOpen={isMaintenanceModalOpen}
            onClose={handleCloseMaintenanceModal}
            vehicle={selectedVehicle}
            onSuccess={handleMaintenanceAdded}
          />
      )}

      {selectedVehicle && (
          <InspectionModal
            isOpen={isInspectionModalOpen}
            onClose={handleCloseInspectionModal}
            car={selectedVehicle}
          />
      )}
    </div>
  );
};

export default CarManagement;